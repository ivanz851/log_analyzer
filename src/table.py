...
class Table:
    ...
    def add_rows(self, new_rows: List[Dict[str, str]]) -> None:
        ...
        new_columns = {key for row in new_rows for key in row.keys()}
    ...
    def get_column_length(self, column: str) -> int:
        ...
        max_length = max(
            (len(str(row.get(column, self.DEFAULT_CELL_VALUE))) if row.get(column) else len(self.DEFAULT_CELL_VALUE)
             for row in self._rows),
            default=0
        )
        return max(max_length, len(column))

    def get_cell(self, row_ind: int, column: str) -> Optional[str]:
        """
